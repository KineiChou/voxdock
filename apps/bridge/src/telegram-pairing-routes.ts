...
import type { TelegramPairingService } from './telegram-pairing-service.js';
const strict = { additionalProperties: false };
const empty = Type.Object({}, strict);
const flowParams = Type.Object({ id: Type.String({ format: 'uuid' }) }, strict);
export function registerTelegramPairingRoutes(app: FastifyInstance, prefix: string, service: TelegramPairingService): void {
  ...
  const base = `${prefix}/connections/telegram`;
