...
The development host had no running Docker daemon during packaging preparation. Compose parsing and config validation are useful static checks, not proof of image build or container operation. See [acceptance](acceptance.md) for open gates and [third-party notices](../THIRD_PARTY_NOTICES.md) for distribution boundaries.
