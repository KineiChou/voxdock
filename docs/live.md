...
On macOS arm64 with Node 24 and FFmpeg 6.0, the corrected path first emitted output after 23 ms in each direction. Final output was exactly 72,000 and 144,000 bytes; largest chunks were 1,024 and 3,840 bytes, and peak playback queue occupancy was 1,920 and 3,840 bytes. These are synthetic smoke observations, not production latency guarantees or perceptual quality measurements. A flag ablation isolated `probesize=32` as sufficient; changing only `analyzeduration` retained the delay, while `avioflags=direct` lost final samples and was excluded. Linux FFmpeg 5.1.x remains to be exercised in CI.
