...
The Live client implements one primary GPT-Live-1 WebSocket session with client delegation. It sends `session.start`, waits for `session.started`, streams raw mono signed PCM16 little-endian at 16 or 24 kHz, and requests `session.close`. The default is 24 kHz with Marin and `store: false`. Authentication stays in the server transport. A controlled 16 kHz session on 2026-09-17 reached ready and closed with complete finalization and one second of reported usage; phone audio and delegation remain separate acceptance gates.

`@voxdock/live` exports `LiveClient`, `LiveEvent`, `LiveTransport`, and `createOpenAITransport`. Construct the transport with an API key, pass instructions and an event callback to the client, then call `start()`. Append audio only after `ready`; use `thinking(content)` for quiet business context and `commentary(content, delegationId)` for results intended to be spoken. A null delegation ID selects general commentary. The caller must keep each append within the documented 500-token limit; the byte/character safety limits here are not a tokenizer. Commentary returns a local event ID. `commentaryAccepted` acknowledges context injection, not completed speech or successful business work.
...
## Streaming resampler smoke

Run `FFMPEG_PATH=/trusted/ffmpeg node --import tsx packages/audio/scripts/streaming-smoke.ts` after installing workspace dependencies. This opt-in check feeds 1.5 seconds of synthetic 440 Hz mono PCM in paced 20 ms frames through the actual persistent resampler in both 48→24 and 24→48 kHz directions. It requires output before input closes and within 500 ms, exact final sample counts, complete final samples, preserved sine phase/order, and successful consumption through the standard 500 ms frame queue. A 10-second watchdog bounds each direction.

On macOS arm64 with Node 24 and FFmpeg 6.0, the corrected path first emitted output after 23 ms in each direction. Final output was exactly 72,000 and 144,000 bytes; largest chunks were 1,024 and 3,840 bytes, and peak playback queue occupancy was 1,920 and 3,840 bytes. The same smoke passes on Linux with FFmpeg 6.1.1 and inside the Debian image with FFmpeg 5.1.9; exact runs are linked in [acceptance](acceptance.md). These are synthetic observations, not phone-call latency guarantees or perceptual quality measurements. A flag ablation isolated `probesize=32` as sufficient; changing only `analyzeduration` retained the delay, while `avioflags=direct` lost final samples and was excluded.


## Pacing drift regression

On 2026-09-17, investigation of a premature generic `audio_failed` termination identified a reproducible scheduling defect in both the standalone pacer and runtime audio loop. Each tick set its next deadline to the actual callback time plus the frame duration, so routine timer lateness accumulated against the source sample clock. A deterministic source supplying one 20 ms frame every 20 ms, with consumer timer ticks every 5.05 ms, overflowed the unchanged 500 ms queue at 50,000.05 ms: 2,500 source frames versus 2,475 emitted frames. The new regression failed the original standalone pacer; a separate duplex runtime regression also failed the original runtime implementation.

The runtime now uses the shared pacer, which advances nominal deadlines for ordinary lateness. A delay of at least one whole frame skips missed slots and restarts pacing from the current time, without emitting catch-up bursts. The same minute-long jitter test passes with at least 2,999 emitted frames and at most two queued frames; the duplex test passes in both directions. The queue limit remains 500 ms. A long-stall regression preserves the one-frame-per-tick behavior. Fixed audio-boundary failure codes replace the ambiguous generic runtime reason.

This confirms and repairs cumulative pacing drift under the tested clock pattern. The earlier call recorded only a generic audio failure, so its exact failed queue or write boundary remains unproven. A subsequent authorized handset retest sustained normal conversation to its 120-second duration limit without an audio error; the phone displayed “Call failed” at termination, so normal handset cleanup remains unresolved. See [the retest evidence](acceptance.md#two-minute-retest-and-incoming-rejection-2026-09-17).
