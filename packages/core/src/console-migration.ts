...
export function migrateConsoleFacts(db: Database.Database): void {
  db.transaction(() => {
    ...
    const insert = db.prepare(
      "INSERT INTO call_facts VALUES (?,?,?,?,?,?,?,?)",
    );
    for (const row of db.prepare("SELECT body FROM calls").all() as {
      body: string;
    }[]) {
      ...
      const events = (
        db
          ...
          .all(call.call_id) as { body: string }[]
      ).map((row) => JSON.parse(row.body) as CallEvent);
      const complete =
        events.length === call.revision &&
        events.every((event, index) => event.call_seq === index + 1);
      const connected = events.find(
        (event) => event.call.state === "connected",
      );
      const connectedIndex = events.findIndex(
        (event) => event.call.state === "connected",
      );
      const previous = events[connectedIndex - 1];
      const observedConnectionStart =
        connected &&
        (complete ||
          (previous &&
            previous.call_seq + 1 === connected.call_seq &&
            ["requested", "dialing", "ringing"].includes(previous.call.state)));
      const uncertain =
        call.state === "uncertain" ||
        events.some((event) => event.call.state === "uncertain");
      insert.run(
        call.call_id,
        call.created_at,
        null,
