...
Current architecture choices and their operating consequences. Platform validation boundaries are listed in [known limitations](limitations.md).
...
| Local stdio MCP over the existing HTTPS API | Reuse call admission and status semantics without exposing another server port. Fixed target scopes and stable agent identities restrict calls and preserve ownership across token rotation; the business backend still owns context and execution. |
...
| Primary GPT-Live-1 WebSocket with one delegation mode | Client mode delegates execution to the external backend; Responses mode uses OpenAI-managed model execution with optional web search. Business permissions remain the backend’s responsibility. Live audio is continuous; Realtime commit/voice-turn commands are not substituted. |
...
| Source distribution and operator-local builds | Platform interoperability and restore/endurance validation remain limited. Original-code MIT licensing is distinct from native dependency redistribution. |

## Packaging provenance

The bridge Dockerfile pins Node `24.21.0-bookworm-slim`, pnpm `10.17.1` and Debian FFmpeg `7:5.1.9-0+deb12u1`. The [official Node image inventory](https://github.com/docker-library/official-images/blob/master/library/node) and [Debian FFmpeg package](https://packages.debian.org/bookworm/ffmpeg) describe those package sources. The optional sidecar builds WaCalls source revision `edeb31f0427aba896639db503153b777a405eccf` plus this repository's patch, using Go 1.26.4 and its locked JavaScript client dependencies. Base image digests and all OS dependency revisions are not a frozen release manifest. A package version pin can fail when a distribution repository retires that version; update it through a reviewed dependency change and rerun image validation.

Local image builds include the service, native binding and streaming resampler. A successful build does not establish real calling or a redistribution license inventory; see [known limitations](limitations.md) and [third-party notices](../THIRD_PARTY_NOTICES.md).
