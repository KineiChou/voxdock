...
| Packaging | Both Docker images built in Linux CI and on an ARM64 server; bridge CLI, native/FFmpeg checks and a paused systemd/Compose deployment pass; private WaCalls starts with an empty account store | Public edge-to-service connectivity after the operator opens the network rule; restore and upgrade with provisioned accounts |
...
The installed containers run as a dedicated non-root UID with read-only root filesystems and restricted persistent mounts. With SELinux enforcing, systemd startup and the bridge health check passed. Direct backend checks returned health 200, unauthenticated capabilities 401 and authenticated capabilities with calling/platforms disabled. The loopback-only WaCalls endpoint returned an empty account list. A separate Nginx edge has a valid domain certificate and reverse-proxy configuration; its private TCP route still requires the operator's network rule, so a publicly reachable application is not claimed. Private addresses, domains and credentials remain in the operator's deployment record, outside this repository.
