...
For Telegram, complete these steps in order:

1. Open [Telegram API development tools](https://my.telegram.org/apps), sign in and create an application. Save its **API ID** and **API hash** in **Telegram application**. These application credentials are required for either login method and are separate from the account authorization.
2. Choose **Get Telegram QR code**. In the calling account's Telegram phone app, open Settings → Devices → Link Desktop Device and scan it. QR codes rotate automatically; expired images disappear. If prompted, enter that Telegram account's two-step verification password. The login attempt expires after three minutes. **Phone number** remains available for code-based login. Cancel before switching methods.
3. In **Receiving account**, choose **Send a code** or **Make a call**. From the separate account that should receive calls, send the displayed exact code to the calling account, or call it after starting verification. Review the detected name and stable user ID, then select **Confirm this account**. The old target remains until confirmation. Telegram usernames and phone numbers appear only when supplied by Telegram; neither is required to identify the target. Manual numeric-ID entry remains an advanced fallback.

Telegram verification calls are declined without answering or starting Live. The verification window lasts three minutes; cancel it to stop early. Leaving the page keeps the attempt open until expiry. Confirmation enables the Telegram channel and target; global calling, Live and the backend must still be configured. The bridge privately persists confirmed entity access hashes alongside the login session so numeric user IDs remain resolvable after restart. Include both files in private backups; Telegram **Disconnect** removes the local session and its peer file.

WhatsApp pairing requires a private WaCalls service built with all five current controlled patches; rebuild the sidecar image when upgrading this feature. An older image without `/api/voxdock/sessions/...` cannot provide this pairing flow. Choose **Configure WhatsApp**, then scan the displayed QR in WhatsApp Linked devices using the account that will call you. Select **Refresh QR code** to request a new code while waiting or scanning. The previous QR is hidden immediately; expired codes are also hidden according to the server-provided expiry. If pairing has ended or failed, select **Connect WhatsApp** to start again. If it is already connected, setup goes straight to pairing the receiving account. From your separate receiving account, send the displayed code to the linked number or use the incoming-call alternative. Review the detected number and select **Confirm this number**; the existing target stays unchanged until confirmation. Closing the setup dialog cancels an active attempt; leaving the page keeps it armed until its displayed expiry. Confirmation saves the target and enables its WhatsApp channel; calling readiness then updates automatically. Global calling, Live credentials and the agent backend must still be configured in Settings. Verification calls are rejected without answering or starting Live; a rejected verification call is expected. The bridge never sends a pairing message or places a pairing call on your behalf. Pairing, account authentication and call readiness are separate states; none establishes a successful phone call. Timeouts/cancellation are reported by the backend, and uncertain cleanup blocks further changes.
...
| `POST /connections/telegram/qr` | `{}`; start Telegram QR authorization using saved application credentials |
| `GET /connections/telegram/setup` | Saved credential/session presence, receiving account and pairing availability; no network login |
| `POST /connections/telegram/target-pairings` | `{ method: "message" \| "call" }`; start a three-minute receiving-account verification window |
| `GET /connections/telegram/target-pairings/{id}` | Expiry, challenge, calling-account profile and observed candidate profile with stable user IDs |
| `POST /connections/telegram/target-pairings/{id}/confirm` | `{ candidate_id }`; persist the observed Telegram target after explicit confirmation |
| `POST /connections/telegram/target-pairings/{id}/cancel` | `{}`; cancel without changing the saved target |
...
| `POST /connections/flows/{id}/refresh` | `{}`; refresh an active WhatsApp QR flow, preserving its ID; an already connected flow stays connected |
...
pnpm voxdock connection connect --channel telegram --method qr --config ./local/voxdock.config.json
...
pnpm voxdock connection refresh --flow FLOW_ID --config ./local/voxdock.config.json
pnpm voxdock connection setup --channel whatsapp --config ./local/voxdock.config.json
pnpm voxdock connection setup --channel telegram --config ./local/voxdock.config.json
pnpm voxdock target-pair start --channel telegram --method message --config ./local/voxdock.config.json
pnpm voxdock target-pair status --channel telegram --flow FLOW_ID --config ./local/voxdock.config.json
pnpm voxdock target-pair confirm --channel telegram --flow FLOW_ID --candidate CANDIDATE_ID --config ./local/voxdock.config.json
```

The private update file contains the PUT body, including `expected_revision` and the complete `settings`; optional `secrets` contains only replacements. Pairing input files contain `{ "phone": "+..." }`, `{ "code": "..." }` or `{ "password": "..." }` as appropriate. `connection code|password|cancel --flow ID` and `connection disconnect --channel telegram|whatsapp` complete the lifecycle. `target-pair cancel --channel telegram|whatsapp --flow ID` cancels receiving-account verification. All `target-pair` commands accept a channel; omitting it retains the original WhatsApp default. `--method call` selects the incoming-call alternative. Telegram QR login accepts no input file; omit `--method` or use `--method phone` for the phone-input flow. Telegram renews its own QR tokens automatically; `connection refresh` is for WhatsApp only. `connection target --channel telegram --input-file FILE` accepts `{ "peer_id": "123456789", "enabled": true }`. The browser and CLI confirm only an opaque candidate ID; they cannot supply an arbitrary phone number or Telegram user ID as observed evidence. `connection unlink --channel whatsapp` performs the same confirmed sign-out; unlike `disconnect`, it requires a fresh QR and recipient verification afterward. Keep credential-bearing files and displayed verification codes private.
