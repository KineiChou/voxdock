...
`GET /settings/options` on both prefixes returns voice/language choices, delegation modes, Responses models, reasoning efforts, service tiers, verbosities and tool choices, with flags for supported custom voice/language/model values. Each choice has a `value` and display `label`. This backend-owned catalog is also available through `voxdock settings options`. It contains suggestions, preserves custom configuration values, makes no provider request, and follows the same authentication, no-store and remote-management rules as other settings routes.

`settings.live` includes voice instructions, a greeting toggle, an optional custom voice resource, and `delegation: client | responses`. Managed Responses settings include the model, separate instructions, reasoning effort, output cap, service tier, verbosity, web search, tool choice and parallel tool use. Client mode keeps execution in the external backend. Responses mode uses the provider directly while retaining the backend context/event contract. See [configuration](configuration.md) for defaults, field boundaries and complete update examples. Managed response results enter the call audit automatically; the external result-callback API remains for client-managed work.
...
Connection flows report `starting`, `code_required`, `password_required`, `qr_required`, `connected`, `cancelled`, `expired` or `failed`. A QR is ephemeral and appears only in `qr_required`. The server chooses the configured platform account and upstream service; clients cannot supply arbitrary service URLs. Calls are never fabricated to test pairing. WhatsApp requires the matching controlled sidecar patches. Each platform has a separate receiving-account flow that exposes waiting/candidate/completed/cancelled/expired/failed, a bounded expiry and an opaque candidate ID. Confirm commits only the provider-observed identity after explicit operator review. Telegram profiles contain `user_id`, `display_name` and optional `username`/`phone`; native access hashes never appear in responses. The [console API](console.md#api-and-cli-parity) lists both browser and bearer routes, with CLI equivalents.

`POST /connections/telegram/qr {}` starts Telegram QR authorization using saved API ID/hash. Poll the shared flow endpoint for `qr` and `qr_expires_at`; the SDK renews individual tokens within the fixed flow deadline. Two-step verification uses the existing `/password` route. CLI: `voxdock connection connect --channel telegram --method qr`. `POST /connections/telegram/target-pairings { method: "message" | "call" }` and its status/confirm/cancel routes verify the receiving account; CLI: `voxdock target-pair start --channel telegram --method message`. Both use exclusive backend management ownership and the same authentication as settings.
